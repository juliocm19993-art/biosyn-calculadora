...
export default function PeptideCalculator() {
  ...
  const [doseDiaria, setDoseDiaria] = React.useState("500");
  const [unidadeDose, setUnidadeDose] = React.useState<UnidadeDose>("mcg");

  const mg = parseValue(mgPeptideo);
  const ml = parseValue(mlAgua);
  const doseValor = parseValue(doseDiaria);
  const mcg = unidadeDose === "mg" ? doseValor * 1000 : doseValor;

  const ui = React.useMemo(() => {
    const doseMg = mcg / 1000;
    if (mg <= 0 || ml <= 0 || doseMg <= 0) return 0;
    return (doseMg * ml * 100) / mg;
  }, [mg, ml, mcg]);

  const uiPorMg = React.useMemo(() => {
    if (mg <= 0 || ml <= 0) return 0;
    return (ml * 100) / mg;
  }, [mg, ml]);
  ...
  const totalDoses = React.useMemo(() => {
    const doseMg = mcg / 1000;
    if (mg <= 0 || doseMg <= 0) return 0;
    return mg / doseMg;
  }, [mg, mcg]);
          ...
          <div className="mb-12 flex justify-center">
            <div className="relative">
              <img
                src="/logo-biosyn.png"
                alt="BioSyn Peptides"
                className="h-40 w-auto object-contain drop-shadow-[0_0_25px_rgba(16,185,129,0.35)] md:h-56"
              />
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-3xl border border-emerald-400/15 bg-white/[0.04] p-6 shadow-2xl backdrop-blur-xl md:p-8">
              ...
              <div className="space-y-5">
                ...
                <div>
                  <label className="mb-2 block text-sm text-slate-300">
                    {unidadeDose.toUpperCase()} - Dose Desejada
                  </label>
                  <div className="relative">
                    <input
                      type="number"
                      step="0.01"
                      value={doseDiaria}
                      onChange={(e) => setDoseDiaria(e.target.value)}
                      className="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-4 pr-28 text-lg outline-none transition focus:border-emerald-400"
                      placeholder={unidadeDose === "mcg" ? "Ex: 500" : "Ex: 0.5"}
                    />

                    <button
                      type="button"
                      onClick={() =>
                        setUnidadeDose(unidadeDose === "mcg" ? "mg" : "mcg")
                      }
                      className="absolute right-3 top-1/2 -translate-y-1/2"
                    >
                      <div className="relative h-[34px] w-[90px] overflow-hidden rounded-full border border-emerald-400/20 bg-black/40 shadow-[0_0_20px_rgba(16,185,129,0.12)] backdrop-blur-md">
                        <div
                          className={`absolute left-[2px] top-[2px] h-[30px] w-[42px] rounded-full bg-gradient-to-r from-emerald-300 to-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.5)] transition-all duration-300 ease-in-out ${
                            unidadeDose === "mg" ? "translate-x-[44px]" : "translate-x-0"
                          }`}
                        />

                        <div className="relative z-10 flex h-full items-center text-[11px] font-semibold tracking-[0.18em]">
                          <span
            ...
            <div className="space-y-6">
              <div className="rounded-3xl border border-emerald-400/20 bg-emerald-500/10 p-6 shadow-2xl md:p-8">
                ...
                <p className="mt-3 text-lg text-slate-200">
                  {format(doseValor)} {unidadeDose} por dia ={" "}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="rounded-3xl border border-emerald-400/15 bg-white/[0.04] p-5 md:col-span-2">
                  <p className="text-sm text-emerald-100/60">
                    Quantidade de doses do frasco
                  </p>
                  <div className="mt-2 text-3xl font-bold">{format(totalDoses)}</div>
                  <p className="mt-2 text-sm text-slate-300">
                    Aproximadamente {format(totalDoses)} aplicações com a dose atual.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-emerald-400/15 bg-white/[0.04] p-6 md:p-8">
              ...
              <div className="space-y-3 leading-relaxed text-slate-300">
                ...
                <p>
                  ...
                  dose diária e escolha a unidade <span className="text-emerald-300">MCG ou MG</span>.
                  Exemplo: 500 mcg ou 0,5 mg.
                </p>
                <p>
                  <span className="font-semibold text-white">4.</span> O sistema
                  converte automaticamente entre MCG e MG e mostra o resultado final
                  em UI.
                </p>
              </div>
